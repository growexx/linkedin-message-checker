...
function Connections() {
  ...
  const [extractFlag, setExtractFlag] = useState(false)
  ...
  const navigateToProfiles = async () => {
    ...
    try {
      ...
      let profile =
        "https://www.linkedin.com/search/results/people/?keywords=tech%20lead&origin=SWITCH_SEARCH_VERTICAL";
      const response = await chrome.tabs.sendMessage(tabDetails.tab.id, {
        message: messages.SCRAPE_DETAILS,
        url: profile,
        csvData: csvData,
      });
  ...
  const navigateAndExtract = async (pageNumber, extractedData) => {
    ...
    try {
      ...
      if (scrollResponse.status === "OK") {
        ...
        await chrome.tabs.sendMessage(tabDetails.tab.id, {
          message: messages.EXTRACTFROMUSERPAGE,
          pageNumber,
        });
    ...
    return (
      <div>
        <div>
          ...
          {/* No of Days: <input type="text" id="name_of_days" />
          <br />
          Total Result: <input type="text" id="total_result" />
          <br />
          Connection to fetch:
          <div>
            <label>
              <input
                type="radio"
                name="radioGroup" // Use the same name for all radio buttons in the group
                value="connected"
                // checked={selectedOption === 'connected'} // Check if this radio button is selected
            ...
            <label>
              <input type="radio" name="radioGroup" value="not_connected" />
              Never Connected
            </label>
          </div> */}
