...
import { messages, columns_name, numbers, date_config, message_config } from '../utils/enum';
...
const checkHeader = (headerContent, details) => {
    if (details.querySelector('.pvs-header__title.text-heading-large')?.children[0].textContent.trim().toLowerCase() === headerContent) {
        return true;
    } else {
        return false;
    }
...
const scrapeSkillFunction = async () => {
    window.scrollTo({ top: document.body.scrollHeight, left: numbers.ZERO, behavior: 'smooth' });
    const outerDiv = document.querySelectorAll('.display-flex.flex-column.full-width.align-self-center');
    let skillFinalArr = [];
    let topSkills = '';
    for (let singleOuterDiv of outerDiv) {
        const skillDivs = singleOuterDiv.querySelector('.display-flex.align-items-center.mr1.hoverable-link-text.t-bold');
        const skillName = skillDivs.querySelector('span.visually-hidden').textContent.trim();
        let numOfEndorsement = numbers.ZERO;
        const isEndorse = singleOuterDiv.querySelector('.hoverable-link-text.display-flex.align-items-center.t-14.t-normal.t-black');
        if (isEndorse) {
...
const scrapePostFunction = async () => {
    ...
    const postList = postSections.querySelectorAll('.profile-creator-shared-feed-update__container');
    for (let list of postList) {
        const repostHtml = list.querySelector('.update-components-header__text-wrapper');
        if (repostHtml) {
            let repostData = repostHtml.querySelector('.update-components-text-view.white-space-pre-wrap.break-words');
            const repostDataText = repostData.querySelector('.update-components-text-view__mention').nextSibling.textContent.trim();
            if (repostDataText && repostDataText.includes('reposted')) {
                break;
            }
        }
        const btnClass = '.feed-shared-inline-show-more-text__see-more-less-toggle.see-more.t-14.t-black--light.t-normal.hoverable-link-text';
        const isShowMoreButton = list.querySelector(btnClass);
        if (isShowMoreButton) {
            const commentaryDiv = list.querySelector(".feed-shared-update-v2__commentary");
            if (commentaryDiv) {
...
const isListElement = (listElement) => {
    if (listElement.tagName === 'UL' || listElement.tagName === 'OL' || listElement.tagName === 'MENU' || listElement.tagName === 'DL') {
        return true;
    } else if (
...
const checkListAttributes = (listElement) => {
    ...
    for (const attribute of listElement.attributes) {
        if ((listAttributes.includes(attribute.name) || attribute.value.includes('list'))) {
            return true;
        }
    }
...
const mainScrapeFunction = async (url) => {
    ...
    window.scrollTo({ top: document.body.scrollHeight, left: 0, behavior: 'smooth' });
    const nameDivs = document.querySelectorAll('span.entity-result__title-text > a > span > span:first-child');
    let names = Array.from(nameDivs).map(x => x.innerText);
    console.log('names--', names);
    let storageNames = document.querySelector(`.${JSON.parse(localStorage.getItem('generalConnections')).profileDetails}`);
...
const msgScrapeFunction = async (url, csvData) => {
    ...
    const msgDivs = document.querySelectorAll('div.msg-conversation-card__row.align-items-center.display-flex');
    const msgConnections = {};
    msgDivs.forEach((element, index) => {
        let date = document.querySelectorAll("time.msg-overlay-list-bubble-item__time-stamp")[index].textContent.trim();
        ...
        msgConnections[index] = {
            'name': document.querySelectorAll("h3.msg-conversation-listitem__participant-names.msg-conversation-card__participant-names")[index].textContent.trim(),
            'time': date,
            'img': document.querySelectorAll('div.msg-selectable-entity.msg-selectable-entity--3 img')[index].src,
            'profile_index': index
        };
    });
...
const filterMessageConnections = async (csvData) => {
    ...
    let msgConnections = JSON.parse(localStorage.getItem('msgConnections')).msgConnections;
    ...
    let filteredArray = totalFilteredArray.slice(0, message_config.TOTAL_CONNECTIONS);
    if (matchConnection.length > 0) {
        if (matchConnection.length < message_config.TOTAL_CONNECTIONS) {
            ...
            let mergedArray = matchConnection.concat(totalFilteredArray.filter(item => !matchConnection.includes(item)));
            filteredArray = mergedArray.slice(0, message_config.TOTAL_CONNECTIONS);
        }

        if (matchConnection.length >= message_config.TOTAL_CONNECTIONS) {
            filteredArray = matchConnection.slice(0, message_config.TOTAL_CONNECTIONS);
        }
    }

    console.log('filteredArray---', filteredArray);
    localStorage.setItem('TotalfilterConnection', JSON.stringify({totalFilteredArray}));
...
const clickMessageBox = (profile) => {
    ...
    window.scrollTo({ top: document.body.scrollHeight, left: 0, behavior: 'smooth' });
    document.querySelectorAll('.msg-overlay-list-bubble__convo-card-content')[profile].click();
...
const inspectList = () => {
    const listElement = document.querySelector(`.${JSON.parse(localStorage.getItem('selectedList')).classList.join('.')}`);
    ...
    if (listItems) {
        highLightListElements(listItems, Number(listElement.getAttribute('list-id')));
    } else {
        highLightListElements(listElement.children, Number(listElement.getAttribute('list-id')));
    }
    detectPagination();
};
...
const triggerClickOnPageBtn = (parentElement, pageNumber) => {
    Array.from(document.getElementsByClassName(parentElement)).forEach((element) => {
...
const extractPageNumber = async () => {
    ...
    //  await delay(1000);
     const pageFrom = 1;
     const pageTo = document.querySelectorAll('.artdeco-pagination__indicator').length;
     console.log(pageFrom, '---', pageTo);
     return [pageFrom, pageTo];
};

const isClassVisible = () => {
    const element = document.getElementsByClassName('block mlA mrA artdeco-button artdeco-button--muted artdeco-button--1 artdeco-button--tertiary ember-view');
    if (element.length > 0) {
...
const scrollMsgConnection = async () => {
    console.log('scrollMsgConnection---');
    let element = document.getElementsByClassName('msg-conversations-container__conversations-list')[0];
    while (true) {
        ...
        console.log('visible---', isSpecificClassVisible);
        console.log('isScrollAtEnd---', isScrollAtEnd);
        if (!isSpecificClassVisible && !isScrollAtEnd) {
            ...
            let isLoadedNext = loadNextMsgConnection();
        } else if (!isSpecificClassVisible && isScrollAtEnd) {
            console.log('completed while loop');
...
const loadNextMsgConnection = async () => {
    console.log('loadNextMsgConnection---');
    let element = document.getElementsByClassName('block mlA mrA artdeco-button artdeco-button--muted artdeco-button--1 artdeco-button--tertiary ember-view')[0];
    element.click();
    await delay(1000);
...
const extractMsgConnection = async () => {
    ...
    let list = document.getElementsByClassName('msg-conversation-card__row msg-conversation-card__title-row');
    const msgDivs = document.querySelectorAll('div.msg-conversation-card__row.align-items-center.display-flex');
    const msgConnections = {};
    list.forEach((element, index) => {
        let date = element.querySelector("time.msg-conversation-listitem__time-stamp").textContent.trim();
        let name = element.querySelector("h3.msg-conversation-listitem__participant-names.msg-conversation-card__participant-names").textContent.trim();
        let img = document.querySelectorAll('div.msg-selectable-entity__entity img')[index].currentSrc;
        if (isNaN(Date.parse(date))) {
            date = new Date();
        }
        if (!/\d{4}/.test(date)) {
...
chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
    ...
    switch (request.message) {
        ...
        case messages.SCROLLMSGCONNECTION: {
            const response = scrollMsgConnection(request);
            if (response) {
        ...
        case messages.EXTRACTMSGCONNECTION: {
            const data = await extractMsgConnection(request);
